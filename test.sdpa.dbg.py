...
# q = torch.ones((Z, H, N_CTX, D_HEAD), device='xpu', dtype=dtype)
# k = torch.ones((Z, H, N_CTX, D_HEAD), device='xpu', dtype=dtype)
...
#torch_output = torch.nn.functional.scaled_dot_product_attention(q, k, v, attn_mask=None, dropout_p=0.0, is_causal=False, scale = sm_scale)
...
print(ref_p)
#p = torch.exp2(p)
#p = torch.dropout(p, p=0.0, train=True)
torch_output = torch.matmul(p, v)

############ for debug
out = triton_output.to("cpu")
out_np = out.to(torch.float16).numpy()
print("out_middle")
for i in range(16):
    print("[", end="")
    for j in range(64):
        ...
        print(r0, ", ", end="")
    print("]")
ref = torch_output.to("cpu")
for i in range(128):
    for j in range(64):
        o0 = out[0][0][i][j]
        # r0 = ref[0][0][i][j]
        r0 = ref[0][0][i][j].to(torch.float32)
        diff = abs(o0 - r0)
        tolerance = abs(r0 / 1000.0) + 0.001
        if diff > tolerance:
            print("error diff at index ", i, j)
            print("out is ", o0)
            print("ref is ", r0)
            print("div is ", o0/r0)
