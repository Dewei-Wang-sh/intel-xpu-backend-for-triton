...
def max(input, axis=None, return_indices=False, return_indices_tie_break_left=True, keep_dims=False, cross_warp=False, dst_warps=()):
    ...
    if return_indices:
        ...
        return core.reduce(input, axis, _elementwise_max, keep_dims=keep_dims, combine_op="max", cross_warp=cross_warp, dst_warps=dst_warps)
...
def sum(input, axis=None, keep_dims=False, cross_warp=False, dst_warps=()):
    input = core._promote_bfloat16_to_float32(input)
    return core.reduce(input, axis, _sum_combine, keep_dims=keep_dims, combine_op="add", cross_warp=cross_warp, dst_warps=dst_warps)
